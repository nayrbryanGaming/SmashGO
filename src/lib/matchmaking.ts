...
import { sendMatchFoundNotification } from './fcm'
...
export async function findMatch(queueEntryId: string): Promise<{
  ...
  // Kirim push notification ke kedua pemain
  const fcmA = (requester.users as any)?.fcm_token
  const fcmB = (bestCandidate.users as any)?.fcm_token
  
  await Promise.all([
    fcmA ? sendMatchFoundNotification(fcmA, {
      matchId: newMatch.id,
      opponentName: (bestCandidate.users as any)?.full_name || 'Lawan',
      opponentElo: bestCandidate.user_elo,
      message: 'Lawan ditemukan! Konfirmasi dalam 5 menit.'
    }) : Promise.resolve(),
    
    fcmB ? sendMatchFoundNotification(fcmB, {
      matchId: newMatch.id,
      opponentName: (requester.users as any)?.full_name || 'Lawan',
      opponentElo: requester.user_elo,
      message: 'Lawan ditemukan! Konfirmasi dalam 5 menit.'
    }) : Promise.resolve()
  ])
  ...
  await Promise.all([
    supabase.from('notifications').insert({
      ...
      body: `Kamu akan bertanding melawan ${(bestCandidate.users as any)?.full_name}. ELO mereka: ${bestCandidate.user_elo}. Konfirmasi sekarang!`,
    ...
    supabase.from('notifications').insert({
      ...
      type: 'match_found',
      title: 'Lawan Ditemukan!',
      body: `Kamu akan bertanding melawan ${(requester.users as any)?.full_name}. ELO mereka: ${requester.user_elo}. Konfirmasi sekarang!`,
...
export async function updateEloAfterMatch(matchId: string): Promise<void> {
  const { data: match } = await supabase
    .from('matches')
    .select('*, player_a:users!matches_player_a_id_fkey(id, elo_rating, total_matches, total_wins, loyalty_points), player_b:users!matches_player_b_id_fkey(id, elo_rating, total_matches, total_wins, loyalty_points)')
    .eq('id', matchId)
    .eq('status', 'completed')
    .single()
  ...
  const { calculateNewElo } = await import('./elo')
  ...
  await Promise.all([
    supabase.from('users').update({
      ...
      loyalty_points: match.player_a.loyalty_points + (isAWinner ? ELO_CONFIG.POINTS_WIN : ELO_CONFIG.POINTS_LOSE)
    }).eq('id', match.player_a_id),
    
    supabase.from('users').update({
      elo_rating: result.playerBNewElo,
      total_matches: match.player_b.total_matches + 1,
      total_wins: !isAWinner ? match.player_b.total_wins + 1 : match.player_b.total_wins,
      loyalty_points: match.player_b.loyalty_points + (!isAWinner ? ELO_CONFIG.POINTS_WIN : ELO_CONFIG.POINTS_LOSE)
